...
function App() {
  ...
  const handleCheck = (category, isChecked) => {
    ...
    if (isChecked) {
      setCheckedItems((prev) => [...new Set([...prev, category.id, ...allChildIds])]);
    } else {
      setCheckedItems((prev) => prev.filter((id) => ![category.id, ...allChildIds].includes(id)));
    }
  };
  ...
  const getDisplayItems = () =>
    checkedItems.map((id) => ({ id, name: getCategoryNameById(id, categoriesData) }));
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {getDisplayItems().map((item) => (
              <span key={item.id} className="px-3 py-1 rounded-full text-sm bg-sky-400 text-white">
                {item.name}
              </span>
            ))}
          </div>
