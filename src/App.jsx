      ...
      <main className="flex-1 p-4 md:p-6">
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {checkedItems
              .filter(id => !categoriesData.some(cat => cat.id === id))
              .map(id => {
                const level = getCategoryLevelById(id, categoriesData);
                return <span key={id} className={`px-3 py-1 rounded-full text-sm ${level === 0 ? "bg-sky-400 text-white" : "bg-sky-400 text-white"}`}>{getCategoryNameById(id, categoriesData)}</span>;
              })}
          </div>
        </div>
      </main>
